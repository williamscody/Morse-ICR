...
//            var recognizerIntent = Intent(RecognizerIntent.ACTION_GET_LANGUAGE_DETAILS)
    ...
    private fun createRecognizer(onDevice: Boolean, listenMode: ListenMode, pauseFor: Int?) {
        // Patched (morse_icr, 2026-09-10, third_party/speech_to_text local
        // fork of upstream 7.4.0): upstream reused the same
        // SpeechRecognizer instance across every startListening() call
        // whenever onDevice matched the previous call (the overwhelmingly
        // common case -- this app always passes the same onDevice value).
        // Confirmed on-device (Moto G Play 2024): a session's *first*
        // listen() against a freshly created recognizer ran cleanly for
        // 5+ seconds and captured genuine speech; every restart after
        // that -- reusing that same instance, per this now-removed
        // early-return -- failed instantly with ERROR_CLIENT
        // ("error_client"), 100% of the time, for the rest of the
        // session. Android's own SpeechRecognizer docs/community
        // consensus: calling startListening() again on an instance that
        // hasn't been freshly created (not just cancel()'d -- that alone
        // was also tried and didn't fix this) is a known cause of
        // exactly this error. Always destroying and recreating below
        // trades a small bit of per-restart overhead (this already
        // happens up to ~3x/second under this app's own restart
        // throttle) for a recognizer that's actually usable past its
        // first session.
